...
/**
 * 撤销 / 重做控制器：把 MainController 里散落的撤销栈交互抽出来。
 *
...
public class UndoController {
    ...
    public UndoController(BookContext ctx, StatusSink status) {
        this(ctx, status, () -> {});
    }
     ...
     */
    static UndoController forTesting(BookContext ctx, StatusSink status,
                                     ValidationClearer validationClearer) {
        return new UndoController(ctx, status, validationClearer, true);
    }

    private UndoController(BookContext ctx, StatusSink status,
    ...
    public void restore(BookHistory.Snapshot snapshot, String message) {
        ...
        ctx.bus().publish(new com.epubra.app.support.AppEventBus.BookRestoredEvent());
